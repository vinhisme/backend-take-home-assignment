    ...
    .mutation(async ({ ctx, input }) => {
      return ctx.db.connection().execute(async (conn) =>
        ...
        conn
          ...
          .innerJoin(
            userMutalFriendCount(conn).as('mutualFriendCount'),
...
const userMutalFriendCount = (db: Database) => {
  return db
    .selectFrom('friendships as fr1')
    .innerJoin('users as user', 'user.id', 'fr1.userId')
    .where('fr1.status', '=', FriendshipStatusSchema.Values['accepted'])
    .select((eb) => [
      'fr1.friendUserId',
      eb.fn.count('fr1.friendUserId').as('mutualFriendCount'),
    ])
    .groupBy('fr1.userId')
}