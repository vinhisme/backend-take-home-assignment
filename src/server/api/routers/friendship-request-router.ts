...
export const friendshipRequestRouter = router({
  send: procedure
    ...
    .mutation(async ({ ctx, input }) => {
      const friendshipRequest = await ctx.db.selectFrom('friendships')
        .select(['friendships.id', 'friendships.status'])
        .where(
          'friendships.status',
          '=',
      ...
      if (friendshipRequest[0]?.status == 'declined') {
        ctx.db.updateTable('friendships')
    ...
    .mutation(async ({ ctx, input }) => {
        const friendshipsstatus = await ctx.db
          .selectFrom('friendships')
          ...
          .limit(1)
          .execute()
        if (ctx.session.userId > input.friendUserId && friendshipsstatus[0]?.status == 'requested') {
          ctx.db.updateTable('friendships')
            .set({ status: 'accepted' })
            .where('friendships.userId', '=', input.friendUserId)
            .where('friendships.friendUserId', '=', ctx.session.userId)
  ...
  decline: procedure
    ...
    .input(AnswerFriendshipRequestInputSchema)
    .mutation(async ({ ctx, input }) => {
      await ctx.db.transaction().execute(
        async (t) => {
          ctx.db.updateTable('friendships')
            .set({ status: 'declined' })
            .where(
